(function () {
  var creatures = {
    ...
    otter: {
      w: 56, h: 28, speed: 9.5, type: "ground",
      paths: [
    ...
    dog: {
      w: 56, h: 38, speed: 6.5, type: "ground",
      paths: [
  ...
  function injectStyles() {
    ...
    s.textContent = [
      ".magical-creature{position:fixed;z-index:1;pointer-events:none;opacity:0;animation-fill-mode:forwards}",
      ".mc-inner{position:relative}",
      ".mc-trail{position:absolute;top:0;left:0;width:100%;height:100%;opacity:0.3;filter:blur(4px)}",
      "@keyframes mc-traverse{0%{transform:translateX(var(--mc-from));opacity:0}5%{opacity:1}95%{opacity:1}100%{transform:translateX(var(--mc-to));opacity:0}}",
      "@keyframes mc-bob{0%,100%{transform:translateY(0)}50%{transform:translateY(-4px)}}",
      "@keyframes mc-gallop{0%,100%{transform:rotate(0deg) translateY(0)}20%{transform:rotate(3deg) translateY(-2px)}50%{transform:rotate(0deg) translateY(1px)}70%{transform:rotate(-3deg) translateY(-2px)}}",
      "@keyframes mc-flap{0%,100%{transform:scaleY(1) translateY(0)}30%{transform:scaleY(0.8) translateY(2px)}60%{transform:scaleY(1.05) translateY(-3px)}}",
      "@keyframes mc-trail-pulse{0%,100%{opacity:0.2}50%{opacity:0.4}}"
    ].join("\n");
  ...
  function spawn(name) {
    ...
    var size = rand(44, 60);
    var scale = size / c.w;
    var isFlying = c.type === "fly";
    var motionAnim = isFlying
      ? "mc-flap 0.5s ease-in-out infinite"
      : "mc-gallop 0.35s ease-in-out infinite";

    // Outer wrapper: traverse across screen + fade in/out
    var wrap = document.createElement("div");
    wrap.className = "magical-creature";
    wrap.style.top = yPct + "vh";
    wrap.style.left = "0";
    wrap.style.width = size + "px";
    wrap.style.color = "rgba(192, 210, 235, 0.18)";
    wrap.style.filter = "drop-shadow(0 0 8px rgba(192, 210, 235, 0.2))";
    ...
    wrap.style.animation = "mc-traverse " + dur.toFixed(1) + "s ease-in-out forwards";

    // Flip wrapper: direction facing
    var flip = document.createElement("div");
    if (goLeft) flip.style.transform = "scaleX(-1)";

    // Bob wrapper: vertical oscillation
    var bob = document.createElement("div");
    bob.style.animation = "mc-bob " + (isFlying ? "1.5" : "2") + "s ease-in-out infinite";

    // Motion wrapper: gallop or flap
    var motion = document.createElement("div");
    motion.style.animation = motionAnim;

    // Main SVG
    var svg = makeSVG(c, size, scale);

    // Ghost trail SVG (blurred duplicate behind)
    var inner = document.createElement("div");
    inner.className = "mc-inner";
    var trail = document.createElement("div");
    trail.className = "mc-trail";
    trail.style.animation = "mc-trail-pulse 1.5s ease-in-out infinite";
    var trailSvg = makeSVG(c, size, scale);
    trail.appendChild(trailSvg);

    inner.appendChild(trail);
    inner.appendChild(svg);
    motion.appendChild(inner);
    bob.appendChild(motion);
