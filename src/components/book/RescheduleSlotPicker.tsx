...
interface Props {
	...
	lang: "vi" | "en";
	title: string;
	// Pre-filled from the original booking — we keep the guest's original tz.
...
export default function RescheduleSlotPicker({
	...
	const t = {
		...
		submit: isVi ? "Xác nhận đổi giờ" : "Confirm reschedule",
		submitting: isVi ? "Đang xử lý…" : "Rescheduling…",
	...
	async function submit(e: React.FormEvent) {
		...
		setError(null);
		try {
			const res = await fetch("/api/weasley-clock/bookings/reschedule", {
				method: "POST",
				headers: { "Content-Type": "application/json" },
				body: JSON.stringify({
					reschedule_token: rescheduleToken,
					new_slot_start_iso: selectedSlot.start_iso,
					guest_timezone: guestTz,
					lang,
				}),
			});
